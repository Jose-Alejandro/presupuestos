...
async function getRTable() {
	let result = await fetch('/budgets/' + budget.id + '/recursos',
		{
			method: 'GET',
			headers: headers
		}
	...
	for (let i = 0; i < res.length; i++) {
		createRRow(table, res[i]);
		total += res[i].amount;
	}
	addTotalRrow(table, total);
};
getRTable();

const createRRow = (table, r) => {
	...
	eliminar.setAttribute("onclick", `deleteR('${r.id}')`);
	row.appendChild(del);
	del.appendChild(eliminar);
};

const addTotalRrow = (table, r) => {
	let row = document.createElement('div');
	row.classList.add('row');
...
let newRButton = document.getElementById('addR');

newRButton.addEventListener('click', async () => {
	try {
		let form = document.forms['newR'];
