...
async function getCDTable() {
	let result = await fetch('/budgets/' + budget.id + '/costosDirectos',
		{
			method: 'GET',
			headers: headers
		}
	...
	for (let i = 0; i < res.length; i++) {
		createCDRow(table, res[i]);
		total += res[i].amount;
	}
	addTotalCDrow(table, total);
};
getCDTable();

const createCDRow = (table, r) => {
	...
	eliminar.setAttribute("onclick", `deleteCD('${r.id}')`);
	row.appendChild(del);
	del.appendChild(eliminar);
};

const addTotalCDrow = (table, r) => {
	let row = document.createElement('div');
	row.classList.add('row');
...
let newCDButton = document.getElementById('addCD');

newCDButton.addEventListener('click', async () => {
	try {
		let form = document.forms['newCD'];
